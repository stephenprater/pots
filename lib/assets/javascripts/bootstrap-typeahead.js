...
function ($) {
    ...
    var Typeahead = function (element, options) {
        ...
        if (!!(this.source.constructor && this.source.call && this.source.apply)) {
          this.source_function = this.source
        }
    ...
    Typeahead.prototype = {
        ...
        //------------------------------------------------------------------
        //
        //  make the data attribute optional for the Typeahead data source
        //  provide "false" to the display options to use the actual value of
        //  of the object for typehead lookup
        //
        dataProperty: function(item, property) {
        ...
        lookup: function (event) {
            ...
            if (that.ajax) {
                ...
                items = that.grepper(that.source_function ? that.source_function() : that.source);
        ...
        grepper: function(data) {
            ...
            if (data && data.length && !that.dataProperty(data[0], that.options.display)) {                
                return null;
            }

            items = $.grep(data, function (item) {
                return that.matcher(that.dataProperty(item,that.options.display), item);
            });
        ...
        sorter: function (items) {
            ...
            while (item = items.shift()) {
                if (!that.dataProperty(item,that.options.display).toLowerCase().indexOf(this.query.toLowerCase())) {
                    beginswith.push(item);
                }
                else if (~that.dataProperty(item,that.options.display).indexOf(this.query)) {
        ...
        render: function (items) {
            ...
            items = $(items).map(function (i, item) {
                ...
                i.find('a').html(that.highlighter(that.dataProperty(item,that.options.display), item));
                return i[0];
            });
        ...
        select: function () {
            ...
            var $selectedObj = {
              'id' : $selectedItem.data('value'),
            }
            $selectedObj[this.options.display] = $selectedItem.text()
            
            this.$element.trigger('typeahead.beforeSelect',[this.$element.val(), $selectedItem.text(), $selectedObj])
            this.$element.val($selectedItem.text()).change();
            this.$element.trigger('typeahead.select', [$selectedItem.text(), $selectedObj])
            return this.hide();
        },
        ...
        keypress: function (e) {
            ...
            switch (e.keyCode) {
                case 13:
                    // enter
                   if (!this.shown) {
                     this.$element.trigger('typeahead.noSelect', this.$element.val() )
                   }
                   e.preventDefault();
                   break;
                case 9:
