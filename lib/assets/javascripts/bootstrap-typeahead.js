...
function ($) {
    ...
    Typeahead.prototype = {
        ...
        select: function () {
            ...
            this.$element.trigger('typeahead.beforeSelect',[$selectedItem.attr('data-value'),$selectedItem.text()])
            this.$element.val($selectedItem.text()).change();
            this.$element.trigger('typeahead.select', [$selectedItem.attr('data-value'), $selectedItem.text()])
            return this.hide();
        },
          ...
          e.preventDefault();
            ...
            switch (e.keyCode) {
                ...
                case 9:
                    ...
                    if (!this.shown) {
                      this.$element.val('')
                      inputs = this.$element.closest('form').find(':tabbable')
                      debugger
                      inputs.eq(inputs.index(this.$element) + 1).focus()
                    } else {
                      this.select();
                    }
                    break;
        ...
        keypress: function (e) {
            ...
            switch (e.keyCode) {
                case 13:
                    // enter
                   if (!this.shown) {
                     console.log('drop down not shown which mean no s')
                       if (this.$element.trigger('typeahead.noSelect', e)) {
                         return false;
                       }
                   }
                case 9:
